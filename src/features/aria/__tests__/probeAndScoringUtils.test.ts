...
describe('probeAndScoringUtils', () => {
  ...
  it('does not treat vignette motion alone as Scenario C commitment-threshold signal', () => {
    const vignetteMotion =
      "Theo leaves when flooded, then comes back — they need to repair by talking it through when he returns.";
  ...
  it('repair-only Theo/Morgan answer does not skip Scenario C threshold forcing (no exit/leave framing)', () => {
    const repair =
      'Both of them have something to own. Theo needs to acknowledge the full pattern. Morgan also needs to look at how they are pursuing. Morgan pursues, Theo withdraws. They are both keeping that loop going.';
    expect(hasScenarioCVignetteCommitmentThresholdSignal(repair)).toBe(false);
  });

  it('vignette threshold signal requires Theo or Morgan named (generic "when to leave" repair talk does not qualify)', () => {
    const repairOnly =
    ...
    expect(
      hasScenarioCVignetteCommitmentThresholdSignal(
        'If Theo keeps leaving and Morgan cannot get a real repair, I would say the relationship is not working.'
      )
    ).toBe(true);
  });
  ...
  it('extractScenario3UserCorpusAfterLastRepairPrompt finds CQ2 when assistant paraphrases the repair question', () => {
    const msgs = [
      { role: 'assistant' as const, content: 'When Theo comes back — what do you make of that?', scenarioNumber: 3 },
  ...
  it('does not treat Scenario C opening ("more personal" teaser) as Moment 4 handoff', () => {
    expect(
      isScenarioCToPersonalHandoffAssistantContent(
        "Here's the third situation — after this we'll move to something more personal. Morgan and Theo have had the same argument."
      )
    ).toBe(false);
  });
  ...
  it('sliceTranscriptBeforeScenarioCToPersonalHandoff drops personal turns still tagged as scenario 3', () => {
    const msgs = [
      { role: 'assistant' as const, content: 'At what point would you say Theo or Morgan...', scenarioNumber: 3 },
      { role: 'user' as const, content: 'Threshold answer about Theo.', scenarioNumber: 3 },
      {
  ...
  it('Sam + dismissed at dinner without referencing the final line does not skip contempt probe', () => {
    expect(
      hasScenarioAQ1ContemptProbeCoverage(
        'Sam felt dismissed at dinner when the call ran long — like the date did not matter.'
      )
    ).toBe(false);
  });
  ...
  it('reference to final line + passive-aggressive only does not skip contempt probe', () => {
    expect(
      hasScenarioAQ1ContemptProbeCoverage(
        "When Sam says you've made that very clear it's passive aggressive — she's not being direct."
      )
    ).toBe(false);
  });
  ...
  it('reference to final line + stating-a-fact minimization does not skip contempt probe', () => {
    expect(
      hasScenarioAQ1ContemptProbeCoverage(
        "Honestly with you've made that very clear I think Sam is just stating a fact about the pattern."
      )
    ).toBe(false);
  });
  ...
  it('reference to final line + harsh/dismissive read skips contempt probe', () => {
    expect(
      hasScenarioAQ1ContemptProbeCoverage(
        "Sam's you've made that very clear is dismissive and harsh — it shuts Reese down and sounds contemptuous."
      )
    ).toBe(true);
  });

  it('long Reese-centered Scenario A Q1 (incl. “Sam was upset” / family framing) still does not skip contempt probe', () => {
    const answer =
      "Reese sounds like someone who has never had to put their partner first. The fact that they couldn't even see why Sam was upset says a lot about their emotional maturity. Some people just aren't capable of prioritizing their relationship over their family of origin and that's a real problem.";
  ...
  it('detects Moment 5 appreciation assistant anchors across framework variants', () => {
    ...
    expect(isMoment5AppreciationAssistantAnchor('Jordan should have said more than congrats.')).toBe(false);
  });
