...
describe('enrichPersonalMomentClosingForTts', () => {
  ...
  it('replaces well-formed reflective closings with neutral when reflections are disabled', () => {
    const good =
  ...
  it('replaces ungrounded model closings with neutral when reflections are disabled', () => {
    const userAnswer =
...
describe('Moment 4 handoff reflections', () => {
  ...
  it('omits dynamic threshold reflection in M4→M5 bundle when reflections are disabled', () => {
  ...
  it('omits reflection for looking-forward-to-dreading tipping-point threshold answers', () => {
    const out = buildMoment4ThresholdAnswerToMoment5Bundle(
      'Matt',
      MOMENT_5_ACCOUNTABILITY_QUESTION_TEXT,
