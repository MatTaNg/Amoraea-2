...
Exception — Scenario B structural Q2: The appreciation question ("what could James have done so Sarah would feel appreciated") is mandatory after Q1 (and after the optional appreciation branch when it fires). Do not skip it because Q1 already mentioned James's alternatives in passing — only skip if the user's immediately preceding answer already fully addressed that exact prompt. **No** mandatory mirror of Q1 before Q2. Do **not** ask "before the fight even started" without the appreciation construct.
...
- Scenario B: Q1; optional appreciation probe only when Q1 had no on-topic engagement with the scenario; mandatory Q2 (what James could have done so Sarah would feel appreciated); Q3 (repair as James); then **BOUNDARY CLOSURE** (segment close + transition, no content reflection) before Scenario C. Do not skip Q2 because Q1 was sophisticated. Before Q2, use **only** a brief acknowledgment ("Got it.", "Makes sense.", "Well done.") — **not** a relational-pattern reflection. Do **not** use "before the fight even started" wording without the appreciation construct.
