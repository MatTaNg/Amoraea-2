...
const stepTransitionStyles = StyleSheet.create({
  overlay: {
...
export const ModalOnboardingFlow: React.FC<ModalOnboardingFlowProps> = ({ onComplete }) => {
  ...
  const goToPrevStep = async () => {
    ...
    const showBusyOverlay = !!user?.id;
    if (showBusyOverlay) setStepTransitionPending(true);
    try {
      if (user?.id) {
        try {
          ...
          console.error('Error saving progress on back:', e);
        }
      }
      setCurrentStep(prevStep);
    } finally {
      stepTransitionLockRef.current = false;
      if (showBusyOverlay) setStepTransitionPending(false);
    }
  };
  ...
  const goToNextStep = async () => {
    ...
    // Use ref to get the latest data (state might be stale)
    const latestData = onboardingDataRef.current;

    // Clear any pending debounced saves to ensure we save immediately
    if (saveTimeoutRef.current) {
      clearTimeout(saveTimeoutRef.current);
    ...
    if (showBusyOverlay) setStepTransitionPending(true);
    try {
      // Save progress with latest data before moving to next step
      if (user?.id) {
        try {
          const saveResult = await modalOnboardingService.saveProgress(user.id, {
            currentStep: nextStep,
            onboardingData: latestData,
          });
          if (!saveResult.success) {
            console.error('Failed to save progress before step change:', saveResult.error);
            // Still proceed to next step even if save fails
          } else {
            console.log('Progress saved successfully before step change:', { 
              from: currentStep, 
              to: nextStep,
          ...
          // Save ALL fields to profile immediately when moving from any step
          ...
          // Map all onboarding data to profile fields (same logic as completeOnboarding)
          ...
          if (latestData.attractedTo && Array.isArray(latestData.attractedTo) && latestData.attractedTo.length > 0) {
            ...
            // Also save to looking_for column when moving from attraction step
            if (currentStep === 'attraction') {
          ...
          if (
            latestData.recreationalDrugsSocial !== undefined &&
            String(latestData.recreationalDrugsSocial).trim() !== ''
          ) {
            (profileUpdates as any).recreationalDrugsSocial = String(
              latestData.recreationalDrugsSocial
            ).trim();
          }
          if (
          ...
          if (Object.keys(profileUpdates).length > 0) {
            ...
            const profileResult = await profilesRepo.updateProfile(user.id, profileUpdates);
  ...
  const handleComplete = async () => {
    ...
    try {
      // Use ref to get latest data (state might be stale)
      const latestData = onboardingDataRef.current;
      
      console.log('Completing onboarding, current data:', {
      ...
      const result = await modalOnboardingService.completeOnboarding(user.id, latestData);
      
      if (!result.success) {
        console.error('Failed to complete onboarding:', result.error);
        // Even if completeOnboarding fails, ensure hasSeenOnboardingIntro is set
        try {
          const { profilesRepo } = await import('@/data/repos/profilesRepo');
          await profilesRepo.updateProfile(user.id, { hasSeenOnboardingIntro: true });
        ...
        // Still call onComplete to allow user to proceed, but log the error
      } else {
        console.log('Onboarding completed successfully, hasSeenOnboardingIntro set to true');
      }
      
      // Wait a moment to ensure profile update is saved
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      onComplete();
    } catch (error) {
      console.error('Error completing onboarding:', error);
      // Even on error, try to set hasSeenOnboardingIntro to prevent infinite onboarding loop
      try {
        const { profilesRepo } = await import('@/data/repos/profilesRepo');
        await profilesRepo.updateProfile(user.id, { hasSeenOnboardingIntro: true });
    ...
    <View style={{ flex: 1 }}>
      ...
      {stepTransitionPending && (
        <View style={stepTransitionStyles.overlay} accessibilityRole="progressbar" accessibilityLabel="Saving progress">
          ...
          <Text style={stepTransitionStyles.overlayHint}>Saving…</Text>
        </View>
      )}
      {currentStep === 'name' && (
        <NameModal
          ...
          onNext={async (picked?: string[]) => {
            ...
            if (attractedTo && attractedTo.length > 0 && user?.id) {
              try {
                ...
                if (mappedAttraction) {
                  await profilesRepo.updateProfile(user.id, { attractedTo: mappedAttraction });
                }
              } catch (error) {
      ...
      {currentStep === 'relationshipStyle' && (
        <RelationshipStyleModal
          ...
          onNext={async () => {
            // Ensure relationship_style is saved immediately when Next is clicked
            const latestData = onboardingDataRef.current;
