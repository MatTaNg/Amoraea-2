...
export const FiltersStep: React.FC<FiltersStepProps> = ({
  ...
  const handleContinue = async () => {
    ...
    const success = await updateProfile({
      matchPreferences: {
        // Convert to miles for storage if user is in metric country
        ...
        genderPreference: filters.genderPreference.length === 3 
          ? "all" 
          : filters.genderPreference.length === 1 
          ? filters.genderPreference[0] 
          : (filters.genderPreference.length > 0 ? filters.genderPreference.join(",") : undefined) as string | undefined,
        ...
        // Only include optional filters if checkboxes are checked
        ...
        ...(filters.useIncomeFilter && {
          incomeRange: [
            parseInt(filters.incomeRangeMinInput || "0") || 0,
            999999, // Max value (not used in UI, but required by type)
          ],
  ...
  return (
    <View>
      ...
      <View style={styles.row}>
        ...
        />
        <Button
          title={saving || config.savingBasic ? "Saving…" : "Continue"}
          disabled={saving || config.savingBasic}
